    ...
    if status and hasattr(status, "text"):
        ...
        if pjg <= 100:
            if client.me.is_premium:
                text = f"""
<blockquote><b>{pong} status akun premium : true {sks}</b>
<b>{tion} limit check : akun anda tidak dibatasi {sks}</b>
<b>{yubot} ubot : {bot.me.mention}</b>
"""
            else:
                text = f"""
<blockquote><b>status akun : beli prem dulu ya</b>
<b>limit check : akun anda tidak di batasi</b>
<b>ubot : {bot.me.mention}</b>
"""
            ...
            if client.me.is_premium:
                text = f"""
<blockquote><b>{pong} status akun premium : true</b>
<b>{tion} limit check : akun anda bermasalah{ggl}</b> 
<b>{yubot} ubot : {bot.me.mention}</b>
"""
            else:
                text = f"""

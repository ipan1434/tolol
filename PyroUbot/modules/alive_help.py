...
async def user_help_inline(client, inline_query):
    ...
    msg = f"<blockquote><b>❀ ᴍᴇɴᴜ ᴇᴍᴀᴋʟᴏ <a href=tg://user?id={inline_query.from_user.id}>{inline_query.from_user.first_name} {inline_query.from_user.last_name or ''}</a>\n  ᴘʀᴇꜰɪx: {' '.join(SH)}</b></blockquote>"
...
async def help_callback(client, callback_query):
    ...
    top_text = f"<blockquote><b>❀ ᴍᴇɴᴜ ᴇᴍᴀᴋʟᴏ <a href=tg://user?id={callback_query.from_user.id}>{callback_query.from_user.first_name} {callback_query.from_user.last_name or ''}</a>\n  ᴘʀᴇꜰɪx: {' '.join(SH)}</b></blockquote>"
    ...
    if mod_match:
        ...
        await callback_query.edit_message_text(
            text=text 
            + '\n<blockquote><b>❀ 𝙐𝙨𝙚𝙧𝙗𝙤𝙩 𝘽𝙮 @XwyWays✅ -<a href="tg://user?id=5082955178">⦪ ᴡᴀʏꜱᴍᴏᴅᴢᴢ</a> </b></blockquote>',
            reply_markup=InlineKeyboardMarkup(button),
